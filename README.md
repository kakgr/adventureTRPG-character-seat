...
キャラクター詳細画面の「ココフォリアにコピー」を押し、ココフォリアの盤面をクリックして貼り付けます。能力値・技能・ダメージボーナスはパラメータ、HP/MP/正気度はステータス、プロフィール・持ち物・タグはキャラクターメモ、技能判定はチャットパレットとして出力します。立ち絵はココフォリア側の画像管理から設定してください。
