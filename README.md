...
キャラクター詳細画面の「ココフォリアにコピー」を押し、ココフォリアの盤面をクリックして貼り付けます。能力値・技能・ダメージボーナスはパラメータ、HP/MP/正気度はステータス、プロフィール・持ち物・タグはキャラクターメモ、技能判定と正気度ロールはチャットパレットとして出力します。能力値判定コマンドは出力しません。正気度ロールの成功率はシナリオ指定のため、出力された `1d100` の結果を判定値と照合してください。立ち絵はココフォリア側の画像管理から設定してください。
