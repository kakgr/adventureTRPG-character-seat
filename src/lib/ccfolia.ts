...
function buildParams(character: CharacterRecord): CocofoliaParam[] {
  ...
  for (const group of SPECIALIZED_SKILLS) {
    for (const skill of data.skills[group.id]) {
      ...
      if (label)
        params.push({
          label,
          value: String(totalSkillValue(skill.value, data.skills.bonuses[group.id][skill.id])),
        });
    }
  }
  ...
  for (const skill of data.skills.custom) {
    ...
    if (label)
      params.push({
        label,
        value: String(totalSkillValue(skill.value, data.skills.bonuses.custom[skill.id])),
      });
  }
...
function buildSkillEntries(character: CharacterRecord) {
  ...
  for (const group of SPECIALIZED_SKILLS) {
    entries.push(
      ...data.skills[group.id]
        .filter((skill: SpecializedSkill) => nonEmpty(skill.specialty))
        .map((skill: SpecializedSkill) => ({
          label: skill.specialty.trim(),
 ...
 */
export function buildCocofoliaCharacter(
  ...
  return {
    ...
    data: {
      ...
      params: [
        ...
        {
          label: "ダメージボーナス",
          value: String(calculateDamageBonus(data.stats, data.statBonuses)),
        },
      ],
    },
  };
