...
const draftKey = (userId: string, characterId: string | null) =>
  `character-draft:${userId}:${characterId ?? "new"}`;
...
export function loadCharacterDraft(
  ...
  try {
    ...
    if (typeof parsed.name !== "string" || !parsed.data || typeof parsed.data !== "object")
      return null;
    return {
      name: parsed.name,
...
export function clearCharacterDraft(
  storage: Pick<DraftStorage, "removeItem"> | null | undefined,
  userId: string,
  characterId: string | null,
) {
  if (!storage) return;
