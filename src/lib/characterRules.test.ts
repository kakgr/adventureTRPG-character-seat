...
describe("character rules", () => {
  ...
  it("prevents stat values below one and budget overflow", () => {
    const next = data();
    next.stats = { ...next.stats, vitality: 19 };
  ...
  it("calculates derived values", () => {
    const next = data();
    next.stats = { ...next.stats, vitality: 4, strength: 5, magic: 5, mental: 5 };
  ...
  it("adds optional stat bonuses without changing the initial stat budget", () => {
    const next = data();
    next.stats = { vitality: 4, strength: 4, magic: 5, speed: 4, mental: 6 };
  ...
  it("sums all common, specialized and custom skills", () => {
    const next = data();
    next.skills.common.athletics = 40;
    next.skills.luck = 90;
    next.skills.weapon.push({ id: "1", specialty: "剣", value: 70 });
    next.skills.magic.push({ id: "3", specialty: "火球", value: 20 });
    next.skills.custom.push({ id: "2", name: "料理", value: 15 });
    next.skills.bonuses.common.athletics = 5;
    next.skills.bonuses.weapon["1"] = 10;
    expect(sumSkillValues(next.skills)).toBe(145);
    expect(remainingSkillPoints(next.skills)).toBe(255);
    expect(
      totalSkillValue(next.skills.common.athletics, next.skills.bonuses.common.athletics),
    ).toBe(45);
