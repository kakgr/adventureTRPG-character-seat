...
export function CharacterEditorPage() {
  ...
  useEffect(() => { if (isNew || !user || !id) return; void characterService.get(id, user.id).then((record) => { setCharacter(record); setName(record.name); setData({ ...record.data, statBonuses: { ...DEFAULT_DATA.statBonuses, ...record.data.statBonuses } }); }).catch((e) => setError(e instanceof Error ? e.message : 'キャラクターを読み込めませんでした')).finally(() => setLoading(false)) }, [id, isNew, user])
  ...
  return <div className="page page-editor world-page world-harbor" style={{ backgroundImage: `linear-gradient(rgba(29, 35, 33, .74), rgba(29, 35, 33, .88)), url(${WORLD_IMAGES.harborCity})` }}>
    ...
    <form onSubmit={save} className="editor-form">
      ...
      <section className="form-section"><SectionTitle title="能力値"/><div className="stat-layout"><div className="stat-list">{(Object.keys(STAT_LABELS) as StatId[]).map((statId) => <StatRow key={statId} id={statId} value={data.stats[statId]} bonus={data.statBonuses?.[statId] ?? 0} onChange={(delta) => update({ ...data, stats: updateStat(data.stats, statId, delta, isNew) })} onBonusChange={(bonus) => update({ ...data, statBonuses: { ...data.statBonuses, [statId]: bonus } })} />)}</div><div className="derived-card"><div className="derived-row"><span>HP</span><strong>{calculateHp(data.stats, data.statBonuses)}</strong></div><div className="derived-row"><span>MP</span><strong>{calculateMp(data.stats, data.statBonuses)}</strong></div><div className={`points-left ${statRemaining === 0 ? 'points-done' : ''}`}><span>残り能力ポイント</span><b>{Math.max(0, statRemaining)} <small>/ 18</small></b></div></div></div></section>
      <section className="form-section"><SectionTitle title="技能"/><div className="skill-total"><span>残り技能ポイント</span><b className={skillRemaining < 0 ? 'negative' : ''}>{skillRemaining} <small>/ {INITIAL_SKILL_POINTS}</small></b></div><SkillTable data={data} update={update} /></section>
...
type SkillTableRow = { key: string; category: string; label: string; value: number; editableLabel: boolean; onLabelChange?: (value: string) => void; onValueChange: (value: number) => void; onDelete?: () => void }

function SkillTable({ data, update }: { data: CharacterData; update: (next: CharacterData) => void }) {
  const rows: SkillTableRow[] = [
    ...COMMON_SKILLS.map((skill) => ({ key: skill.id, category: '通常', label: skill.label, value: data.skills.common[skill.id], editableLabel: false, onValueChange: (value: number) => updateSkill(data, update, value, data.skills.common[skill.id], (nextSkillValue) => ({ ...data, skills: { ...data.skills, common: { ...data.skills.common, [skill.id]: nextSkillValue } } })) })),
    ...SPECIALIZED_SKILLS.flatMap((group) => data.skills[group.id].map((skill) => ({ key: skill.id, category: group.label, label: skill.specialty, value: skill.value, editableLabel: true, onLabelChange: (label: string) => update({ ...data, skills: { ...data.skills, [group.id]: data.skills[group.id].map((item) => item.id === skill.id ? { ...item, specialty: label } : item) } }), onValueChange: (value: number) => updateSkill(data, update, value, skill.value, (nextSkillValue) => ({ ...data, skills: { ...data.skills, [group.id]: data.skills[group.id].map((item) => item.id === skill.id ? { ...item, value: nextSkillValue } : item) } })), onDelete: () => update({ ...data, skills: { ...data.skills, [group.id]: data.skills[group.id].filter((item) => item.id !== skill.id) } }) }))),
    ...data.skills.custom.map((skill) => ({ key: skill.id, category: 'カスタム', label: skill.name, value: skill.value, editableLabel: true, onLabelChange: (label: string) => update({ ...data, skills: { ...data.skills, custom: data.skills.custom.map((item) => item.id === skill.id ? { ...item, name: label } : item) } }), onValueChange: (value: number) => updateSkill(data, update, value, skill.value, (nextSkillValue) => ({ ...data, skills: { ...data.skills, custom: data.skills.custom.map((item) => item.id === skill.id ? { ...item, value: nextSkillValue } : item) } })), onDelete: () => update({ ...data, skills: { ...data.skills, custom: data.skills.custom.filter((item) => item.id !== skill.id) } }) })),
  ]
  const addSpecialized = (id: SpecializedSkillId) => update({ ...data, skills: { ...data.skills, [id]: [...data.skills[id], { id: newId(), specialty: '', value: 0 }] } })
  const addCustom = () => update({ ...data, skills: { ...data.skills, custom: [...data.skills.custom, { id: newId(), name: '', value: 0 }] } })
  return <div className="skill-table-panel"><div className="skill-table-wrap"><table className="skill-table"><thead><tr><th>区分</th><th>技能名</th><th>技能値</th><th aria-label="操作" /></tr></thead><tbody>{rows.map((row) => <tr key={row.key}><td><span className="skill-category-chip">{row.category}</span></td><td>{row.editableLabel ? <input aria-label={`${row.category}技能名`} value={row.label} onChange={(event) => row.onLabelChange?.(event.target.value)} placeholder="技能名" /> : <b className="skill-name">{row.label}</b>}</td><td><div className="skill-value-input"><NumericInput aria-label={`${row.label || row.category}技能値`} min={0} max={MAX_SKILL_VALUE} value={row.value} onChange={row.onValueChange} /><span>/ 100</span></div></td><td>{row.onDelete && <button type="button" className="text-button danger" onClick={row.onDelete}><Icon name="trash" />削除</button>}</td></tr>)}</tbody></table></div><div className="skill-add-actions">{SPECIALIZED_SKILLS.map((group) => <button type="button" className="add-link" key={group.id} onClick={() => addSpecialized(group.id)}><Icon name="plus" />{group.label}を追加</button>)}<button type="button" className="add-link" onClick={addCustom}><Icon name="plus" />カスタム技能を追加</button></div></div>
}
