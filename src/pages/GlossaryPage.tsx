  ...
  return (
    <>
      {subgroups.map((subgroup) => (
        <div className="glossary-subgroup" key={subgroup.title || "uncategorized"}>
      ...
      <div className="page world-page world-glossary-page" style={glossaryBackground}>
        <section className="world-hero page-intro">
          <div>
            ...
            <Link className="back-link" to="/world/glossary">
              用語帳のジャンル一覧へ
            </Link>
          </div>
        </section>
          ...
          {groupGlossaryEntries(genre.entries).map((group, groupIndex) => (
            <section className="glossary-entry-group" key={group.title}>
              ...
              {group.entries.length ? (
                <GlossaryEntries entries={group.entries} />
              ) : (
                <p className="glossary-group-empty">この小分類の用語はまだありません。</p>
              )}
            </section>
          ))}
        </div>
